...
mean_sorted <- t_tests[order(-t_tests[5,])]
...
#par(mar=c(5,5,4,8))
#barplot(as.matrix(conserved_variable_means),ylim=c(0,7), col=c("black","darkgrey"), beside=T, names.arg=colnames(conserved_variable_means), xlab="Amino Acid", ylab="Mean Count Per Exon", legend = rownames(conserved_variable_means), args.legend = list(x ='topright', bty='n', inset=c(0.035,0.001)))

#dev.off()

# Order into P, E, V, K
ordered_cols <- c(1,3,4,2)
conserved_variable_means_ordered = conserved_variable_means[1:2,][,ordered_cols]

pdf(file="amino_acid_stats.pdf", width=15, height=10, pointsize=22)

par(mar=c(5,5,4,8))
barplot(as.matrix(conserved_variable_means_ordered),ylim=c(0,7), col=c("black","darkgrey"), beside=T, names.arg=colnames(conserved_variable_means_ordered), xlab="Amino Acid", ylab="Mean Count Per Exon", legend = rownames(conserved_variable_means_ordered), args.legend = list(x ='topright', bty='n', inset=c(0.0001,0.006)))
