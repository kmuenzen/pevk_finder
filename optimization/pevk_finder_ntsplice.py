...
for f in frames:
    ...
    def find_pevk(seq, seq_start, seq_end):
        ...
        if identification == "frame_f1":
            nuc_ref_start = int(seq_start*3)
            nuc_ref_end = int(seq_end*3)
        if identification == "frame_f2":
            nuc_ref_start = int(seq_start*3 + 1)
            nuc_ref_end = int(seq_end*3 + 1)
        if identification == "frame_f3":
            nuc_ref_start = int(seq_start*3 + 2)
            nuc_ref_end = int(seq_end*3 + 2)
                ...
                for j in range(0, len(groupings)):
                    ...
                    # If there is a donor site:
                        ...
                        if acceptor_sites != []:
                            ...
                            final_seq = seq[int(relative_seq_start):int(relative_seq_end)]
...
############ END OF NEW CODE 6/12/17 ###########
                    ...
                    if len(pevk_seqs) > 1:
                        ...
                        if range_result != []:
                            for i in range(0, len(range_result)):
                                ...
                                current_seq = str(prot_seq[int(current_start):int(current_stop)])
                        ...
                        if result_final != []:
                            ...
                            for i in range(0, len(result_final)):
                                ...
                                if len(result_final[i][0]) >= int(min_length):
                                    ...
                                    final_seq = prot_seq[int(prot_start):int(prot_end)]
                                    ...
                                    if identification == "frame_f1":
                                        ...
                                        nuc_end = prot_end*3
                                        nt_seq = str(nucleotide_seq[int(nuc_start):int(nuc_end)]) #extract the full nucleotide sequence defined by that region
    ...
    # Create a csv file that contains the exon names and their corresponding lengths
    with open('test_names_and_lengths'+"_"+length+"_"+in_ratio+"_"+min_length+".csv", 'w') as csv_file:
